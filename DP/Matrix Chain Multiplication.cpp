...
Your Task:
You do not need to take input or print anything. Your task is to complete the function matrixMultiplication() which takes the value N and the array arr[] as 
input parameters and returns the minimum number of multiplication operations needed to be performed.
...
public:
    ...
    int rec(int i, int j, int arr[])
    {
        ...
        for(int k = i; k < j; ++k)
        {
            int tempAns = rec(i, k, arr) + rec(k+1, j, arr) + arr[i-1]*arr[j]*arr[k];
            ans = min(ans, tempAns);
        }
        return dp[i][j] = ans;
    }
