...
Note: The Input/Output format and Example given are used for the system's internal purpose, and should be used by a user for Expected Output only. As it is a 
function problem, hence a user should not read any input from the stdin/console. The task is to complete the function specified, and not to write the full code.
*/
...
vector <int> bottomView(Node *root)
{
    ...
    int left_bound = 1, right_bound = 0;
    ...
    while(!q.empty())
    {
        ...
        {
            left[abs(level)] = curr->data; // replace with new value
        }
        if(level > right_bound)
        {
            right_bound = level;
        ...
        {
            right[level-1] = curr->data; // replace with new value
        }
