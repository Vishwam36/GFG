...
// Easy
// Advanced version problem is 4sum on leetcode
...
You just need to complete the function countTriplets() that take array arr[], integer N  and integer X as parameters and returns the count of triplets.
	...
	long long countTriplets(long long arr[], int n, long long sum)
	{
	    ...
	    for(int i = 0; i < n-2; ++i)
	    {
	        ...
	        while(j < k)
	        {
	            ...
	            {
	                ans += k-j; // elements A[j+1] to A[k] all will follow
	                ++j;
	            }
